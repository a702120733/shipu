...
export default function LoadingAnimation({ onComplete, isPageTransition = false }: LoadingAnimationProps) {
  const [progress, setProgress] = useState(0)
  const [isDrawingComplete, setIsDrawingComplete] = useState(false)
  const [isFilling, setIsFilling] = useState(false)
  const pathRef = useRef<SVGPathElement>(null)
  const progressTimerRef = useRef<NodeJS.Timeout>()
  const fillTimerRef = useRef<NodeJS.Timeout>()
  ...
  useEffect(() => {
    ...
    const increment = isPageTransition ? 4 : 2
    const interval = isPageTransition ? 15 : 30
  ...
  // 当描边完成时触发填充和完成回调 - 修复依赖数组问题
  useEffect(() => {
    if (isDrawingComplete && !isFilling && !hasCalledComplete.current) {
      console.log("Starting fill animation")
      setIsFilling(true)

      const delay = isPageTransition ? 250 : 500
      console.log(`Setting timeout for onComplete with delay: ${delay}ms`)

      fillTimerRef.current = setTimeout(() => {
        console.log("Fill animation complete, calling onComplete")
        hasCalledComplete.current = true
        if (onCompleteRef.current) {
          onCompleteRef.current()
        } else {
          console.warn("onComplete callback is undefined")
        }
      }, delay)
    }
  }, [isDrawingComplete]) // 只依赖 isDrawingComplete
  ...
  // Logo尺寸
  const logoWidth = 1426
  const logoHeight = 660
  const scaleFactor = 0.25 * 0.3 * 2
  const strokeWidth = 6
      ...
      <div className="relative">
        ...
        >
          ...
          <path
            ...
            fill={isFilling ? "#3DB7EA" : "none"}
            style={{
              strokeDashoffset: `${pathRef.current ? pathRef.current.getTotalLength() * (1 - progress / 100) : 0}`,
              transition: "stroke-dashoffset 0.05s ease, fill 0.5s ease",
            }}
          />
